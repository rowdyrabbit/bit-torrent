...
class TorrentClientTest extends FlatSpec with Matchers {
  ...
  "The 'bdecode' function" should "return a nested map" in {
    ...
    val output = TorrentClient.bdecode(input)(0)
        ...
        map.get("other") match {
          case Some(nestedMap) => {
            ...
            case n: Map[String, Any] => {
              n.get("announce") should be(Some("http://smh.com.au/"))
            }
          }
        }
      }
  ...
  "The 'bdecode' function" should "return a list of a map" in {
    ...
    output match {
      case map: Map[String, Any] => {
        map.get("announce") should be (Some("http://thomasballinger.com:6969/announce"))
