...
object BTClient {
  ...
  def sha1(s: String): Array[Byte] = {
    println("SHAAAA")
    println("thing we are about to sha1: " + s)
...
//    val lines: Array[Char] = Stream.continually(is.read).takeWhile(-1 !=).map(_.toChar).toArray
//    source.close()
...
//    source.close()
    ...
    Bencode.decode(metainfo.toCharArray)(0)
  }
